  ...
  addCallIntakeFormData() async {
    IntakeModel intakeModel = isEnterPartnerDetails == true
        ? IntakeModel(
            ...
            birthTime: birthTimeController.text.toUpperCase().contains('AM') ||
                    birthTimeController.text.toUpperCase().contains('PM')
                ? DateFormat.Hm()
                    .format(DateFormat.jm().parse(birthTimeController.text))
                    .toString()
                : (DateFormat.Hm()
                    .parse(birthTimeController.text)
                    .toString()), //for 24 hours
            ...
            topicOfConcern: dropDownController.topic ?? 'Present',
            latitude: lat,
            longitude: long,
            timezone: tzone,
          )
          ...
          await apiHelper.getIntakedata().then((result) {
            if (jsonDecode(result)['status'].toString() == "200") {
              ...
              if (intakeData.isNotEmpty) {
                ...
                String displayDate =
                    DateFormat('MM-dd-yyyy').format(intakeData[0].birthDate!);
                selctedDate = DateFormat('MM-dd-yyyy').parse(displayDate);
                birthTimeController.text =
                    intakeData[0].birthTime.toString() == "null"
                        ? "10:10 AM"
                        : DateFormat.jm().format(DateFormat.Hms()
                            .parse(intakeData[0].birthTime.toString()));
                ...
                print("maritial status ${intakeData[0].maritalStatus}");
                dropDownController.maritalStatusChoose(
                    intakeData[0].maritalStatus ?? 'Single');
