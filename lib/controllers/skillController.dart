...
class SkillController extends GetxController {
  ...
  addFilter({List<int>? skills, List<int>? language, List<String>? gender, String? sortBy, bool isLazyLoading = false, int? catId}) async {
    try {
      ...
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.getAstrologer(sortingKey: sortBy, skills: skills, language: language, gender: gender, startIndex: bottomNavigationController.startIndex, fetchRecords: fetchRecord, catId: null).then((result) {
            if (result.status == "200") {
