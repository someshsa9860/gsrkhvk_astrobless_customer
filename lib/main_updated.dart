...
String? _extractFcmImageUrl(RemoteMessage message) {
  final data = message.data;

  final candidate = message.notification?.android?.imageUrl ??
      message.notification?.apple?.imageUrl ??
      data['image'] ??
      data['imageUrl'] ??
      data['image_url'] ??
      data['fcm_options.image'] ??
      data['gcm.notification.image'];
...
Future<String?> _downloadImageForNotification(String imageUrl) async {
  try {
    ...
    final extension = parsedUrl != null && parsedUrl.pathSegments.isNotEmpty
        ? parsedUrl.pathSegments.last.split('.').last
        : 'jpg';
    final safeExtension =
        RegExp(r'^[a-zA-Z0-9]{2,5}$').hasMatch(extension) ? extension : 'jpg';
    final filePath =
        '${tempDir.path}/fcm_bg_${DateTime.now().microsecondsSinceEpoch}.$safeExtension';
...
Future<void> _showBackgroundNotification(RemoteMessage message) async {
  ...
  final imageUrl = _extractFcmImageUrl(message);
  final downloadedPath = null;
  // imageUrl != null ? await _downloadImageForNotification(imageUrl) : null;
  ...
  final androidDetails = AndroidNotificationDetails(
    ...
    styleInformation: downloadedPath != null
...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  log(
    "_firebaseMessagingBackgroundHandler a background message: ${message.data}",
  );
  ...
  if (global.sp!.getString("currentUser") != null) {
    ...
    if (rawBody is String && rawBody.isNotEmpty) {
      try {
        final decodedBody = json.decode(rawBody);
        if (decodedBody is Map<String, dynamic>) {
          messageData = decodedBody;
        }
      } catch (_) {}
    }
...
class _MyAppState extends State<MyApp> {
  ...
  void showForegroundNotification(Map<String, dynamic>? additionalData) async {
    ...
    if (additionalData['title'] != null || additionalData['body'] != null) {
      ...
      // ✅ ADD THIS
      final imageUrl = additionalData['image'] ??
          additionalData['imageUrl'] ??
          additionalData['image_url'];

      String? downloadedPath;
      if (imageUrl != null && imageUrl.toString().isNotEmpty) {
        // downloadedPath = await _downloadImageForNotification(imageUrl);
      }
      ...
      await _localNotifications.show(
        ...
        body,
        NotificationDetails(android: androidDetails, iOS: iosDetails),
        payload: json.encode(additionalData),
      );
      if (body is String) {
        try {
