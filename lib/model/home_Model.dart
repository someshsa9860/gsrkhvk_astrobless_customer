...
HomeModel welcomeFromMap(dynamic str) => HomeModel.fromMap(json.decode(str));

dynamic welcomeToMap(HomeModel data) => json.encode(data.toMap());
...
class HomeModel {
  ...
  factory HomeModel.fromMap(Map<String, dynamic> json) => HomeModel(
        ...
        astrotalkInNews: List<AstrotalkInNews>.from(
            json["astrotalkInNews"].map((x) => AstrotalkInNews.fromJson(x))),
        astrologyVideo: List<AstrologyVideo>.from(
            json["astrologyVideo"].map((x) => AstrologyVideo.fromJson(x))),
        status: json["status"],
        topOrders: json["topOrders"] == null
            ? []
            : List<TopOrder?>.from(
                json["topOrders"]!.map((x) => TopOrder.fromJson(x))),
      );
  ...
  Map<String, dynamic> toMap() => {
        ...
        "topOrders": topOrders == null
            ? []
            : List<dynamic>.from(topOrders!.map((x) => x!.toMap())),
      };
}
...
class AstrotalkInNews {
  ...
  factory AstrotalkInNews.fromJson(Map<String, dynamic> json) =>
      AstrotalkInNews(
        id: json["id"],
...
class Banners {
  ...
  factory Banners.fromJson(Map<String, dynamic> json) => Banners(
        ...
        fromDate: DateTime.parse(
            json["fromDate"] ?? DateTime.now().toIso8601String()),
        toDate:
            DateTime.parse(json["toDate"] ?? DateTime.now().toIso8601String()),
