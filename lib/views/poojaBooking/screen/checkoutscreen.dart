...
class _CheckoutscreenState extends State<Checkoutscreen> {
  ...
  void initState() {
    super.initState();
    log('data booking price is 2->  ${widget.package?.packagePrice}');
    log('image url ${widget.poojadetail?.pujaImages?[0]}  ');
    gst = (double.parse(widget.package!.packagePrice!.toString())) *
        double.parse(global.getSystemFlagValue(global.systemFlagNameList.gst)) /
        100;

    totalamount = double.tryParse(
        ((double.parse(widget.package!.packagePrice!.toString()) +
                  ...
                  try {
                    ...
                    if (value['status'] == 200) {
                      ...
                      if (value['message'] == "Pay Online.") {
                        final data = value['data']; // (we will fix API next)
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              '${global.getSystemFlagValueForLogin(global.systemFlagNameList.currency)} ${widget.package!.packagePrice!.toString()}',
                              style: Get.textTheme.titleMedium,
                            ),
                          ],
                        ),
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              '${global.getSystemFlagValueForLogin(global.systemFlagNameList.currency)} ${(double.parse(widget.package!.packagePrice.toString())).toStringAsFixed(2)}',
                              style: Get.textTheme.titleMedium!.copyWith(
                                  fontSize: 17.sp,
                                  color: Color(0xff79b4fd),
  ...
  Container _buildbannerwidget() {
    return Container(
      ...
      child: ClipRRect(
        borderRadius: BorderRadius.circular(10),
        child: Image.network(
          "${widget.poojadetail!.pujaImages![0]}",
          fit: BoxFit.fill,
        ),
      ),
    );
